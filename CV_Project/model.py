...
# ==============================================================================
# SegNet 模型定义 (从你提供的代码修改)
# ==============================================================================
class SegNet(nn.Module):
    def __init__(self, in_chn=3, out_chn=32, BN_momentum=0.1): # BN_momentum 默认值通常是 0.1
        ...
        self.ConvDe51 = nn.Conv2d(512, 512, kernel_size=3, padding=1) # SegNet 论文中解码器第一层输出通道数与编码器对应层不同，这里保持512
        ...
        # 最后一层通常不加 BatchNorm 和 ReLU，直接输出 logits
    ...
    def forward(self, x):
        ...
        x, ind5 = self.MaxEn(x) # size5 = x.size() # size5 is not used for output_size in MaxUnpool

        # DECODE
        x = self.MaxDe(x, ind5, output_size=size4) # Use size of feature map before corresponding MaxPool
        ...
        x = self.MaxDe(x, ind1) # MaxUnpool to original size before first MaxPool
        x = F.relu(self.BNDe12(self.ConvDe12(x)))
        
        # 移除最后的BN和ReLU，直接输出logits
        x = self.ConvDe11(x)
        
        # **重要: 移除最后的 F.softmax(x, dim=1)**
        # x = F.softmax(x, dim=1) # CrossEntropyLoss 会自己处理 softmax
